...
*/
app.locals.store = new Map();
app.post("/cache/set", express_1.default.json({ type: "application/json" }), async (req, res) => {
    try {
...
app.post("/cache/get", express_1.default.json({ type: "application/json" }), async (req, res) => {
    try {
...
app.delete("/cache/delete", express_1.default.json({ type: "application/json" }), async (req, res) => {
    try {
...
async function deletePurchaseMessage(client, userId, situation) {
    ...
    try {
        ...
        switch (situation) {
            case 'success':
                await dmChannel.send({ embeds: [payment_info_1.PaymentInfoEmbed.setTitle('✅ Pagamento realizado com Sucesso!').setDescription('Muito obrigado por se juntar a nós, agradecemos a preferência! 😉\n\n Aproveite todo o conteúdo disponível no nosso servidor.').setFooter({ text: 'Qualquer problema, contate o suporte!' })] });
                break;
            case 'expired':
                await dmChannel.send({ embeds: [error_1.ErrorEmbed.setTitle('❌ Pagamento Expirado!').setDescription('Você demorou muito para realizar o pagamento, tente gerar outro link pelo servidor.').setFooter({ text: 'Qualquer problema, contate o suporte!' })] });
                break;
        }
    }
    catch (error) {
...
app.post("/webhook", express_1.default.raw({ type: "application/json" }), async (req, res) => {
    ...
    switch (event.type) {
        case "checkout.session.completed":
            ...
            if (userId) {
                try {
                    ...
                    console.log(`Cargo ${role?.name} foi adicionado com sucesso para ${member.user.tag}`);
                }
                catch (error) {
        ...
        case "customer.subscription.deleted":
            ...
            let email_deleted;
            const stripeId_deleted = session_deleted.customer;
            if (stripeId_deleted) {
                try {
                    const cliente = await stripe.customers.retrieve(stripeId_deleted);
            ...
            await channel.send({
                embeds: [
                    error_1.ErrorEmbed.setDescription(`**Usuário:** ${username_deleted}\n**ID:** ${userId_deleted}\n**Email:** ${email_deleted}\n**ID da Stripe:** ${stripeId_deleted}`).setTitle('❌ Cancelamento de Assinatura')
                ]
            });
            if (userId_deleted) {
                try {
                    ...
                    console.log(`Cargo ${role?.name} foi removido com sucesso para ${member.user.tag}`);
                }
                catch (error) {
        ...
        case 'checkout.session.expired':
            ...
            const userId_expired = session_expired.metadata?.userId;
            await deletePurchaseMessage(client, userId_expired, 'expired');
            break;
    }
    res.json({ received: true });
});
app.post("/create-checkout-session", express_1.default.json({ type: "application/json" }), async (req, res) => {
